...
public class Request {
    ...
    public static String getRequestType(String[] data){
        ...
        if(data.length < 2)
            ...
            if(data[2].compareTo("info")==0 && data.length ==3)ans =  data[1] +" "+data[2];//balance info
            else if( data.length ==4 && data[2].compareTo("add")==0  && data[3].matches("\\d+"))ans =  data[1]+" " + data[2];
        }
        else if (data[1].compareTo("info") == 0 ){//info
            ans =  data[1];
        }
        else if(data[1].compareTo("rent") == 0){//rent
            ans = data[1];
        }
        else if(data[1].compareTo("return") == 0){//return
            ans = data[1];
        }
